<div align="center">
  <img src="./src/img/favicon.png" alt="logo"/>
</div>
<h1 align="center">Forky App 🍴</h1>
<div align="center">
  <img 
    src="https://img.shields.io/badge/html5-%23E34F26.svg?style=for-the-badge&logo=html5&logoColor=white" 
    alt="HTML5"
  />
  <img 
    src="https://img.shields.io/badge/css3-%231572B6.svg?style=for-the-badge&logo=css3&logoColor=white" 
    alt="CSS3"
  />
  <img 
    src="https://img.shields.io/badge/javascript-%23323330.svg?style=for-the-badge&logo=javascript&logoColor=%23F7DF1E" 
    alt="JavaSript"
  />
  <img 
    src="https://img.shields.io/badge/SASS-hotpink.svg?style=for-the-badge&logo=SASS&logoColor=white" 
    alt="SASS"
  />
  <img 
    src="https://img.shields.io/badge/Udemy-A435F0?style=for-the-badge&logo=Udemy&logoColor=white" 
    alt="UDEMY"
  />
</div>

Forky App is a web application project developed as part of the Udemy course The Complete JavaScript Course 2025: From Zero to Expert! by Jonas Schmedtmann.

This project serves as a hands-on learning exercise to strengthen my understanding of modern JavaScript. The primary focus is on asynchronous JavaScript and the behavior of the this keyword in different execution contexts. Through this project, I aim to build a strong foundation in JavaScript that will support my transition into Node.js and backend development in the future.

## Key Learnings

- Asynchronous JavaScript (Promises, async/await, and the event loop)
- Understanding and applying the this keyword in various contexts
- Consuming and handling data from external APIs
- Managing asynchronous application state
- Applying the MVC (Model-View-Controller) architectural pattern

## Tech stack

| Technology                                                                                                                   | Description                                                            |
| ---------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| ![HTML5](https://img.shields.io/badge/html5-%23E34F26.svg?style=for-the-badge&logo=html5&logoColor=white)                    | Semantic markup and application structure                              |
| ![CSS](https://img.shields.io/badge/css3-%231572B6.svg?style=for-the-badge&logo=css3&logoColor=white)                        | Styling, layout, and responsive design                                 |
| ![SASS](https://img.shields.io/badge/SASS-hotpink.svg?style=for-the-badge&logo=SASS&logoColor=white)                         | CSS preprocessor for modular and maintainable styles                   |
| ![JAVASCRIPT](https://img.shields.io/badge/javascript-%23323330.svg?style=for-the-badge&logo=javascript&logoColor=%23F7DF1E) | Core application logic, asynchronous programming, and state management |
| PARCEL                                                                                                                       | Module bundler for development and production builds                   |

## Notes of courses

| No  | Topic                                                                                     | Status       |
| --- | ----------------------------------------------------------------------------------------- | ------------ |
| 1   | [Loading a recipe form api](./notes/01-loadig-a-recipe-from-api.md)                       | ✅ Completed |
| 2   | [Rendering the recipe](./notes/02-rendering-the-recipe.md)                                | ✅ Completed |
| 3   | [Listening load and hash change](./notes/03-listening-load-and-hash-change.md)            | ✅ Completed |
| 4   | [The MVC architecture](./notes/04-the-mvc-architecthure.md)                               | ✅ Completed |
| 5   | [Helper and configuration files](./notes/05-helper-and-configuration-function.md)         | ✅ Completed |
| 6   | [Event handler in MVC](./notes/06-event-handler.md)                                       | ✅ Completed |
| 7   | [Implement error and success messages](./notes/07-implement-error-and-success-message.md) | ✅ Completed |
| 8   | [Implement Search result](./notes/08-impplement-search-result.md)                         | ✅ Completed |
| 9   | [Implement pagination](./notes/08a-implement-pagination.md)                               | ✅ Completed |
| 10  | [Updating recipe](./notes/09-updating-recipe.md)                                          | ✅ Completed |
| 11  | [Developing a DOM alghoritmh](./notes/10-developing-dom-alghortihm.md)                    | ✅ Completed |
| 12  | [Implement Bookmark](./notes/11-implement-bookmark.md)                                    | ⏳           |
| 13  | [Stroing bookmark with localstorage](./notes/12-storing-bookmark-with-localstorage.md)    | ⏳           |
| 14  | [Updating a new recipe](./notes/13-updating-new-recipe.md)                                | ⏳           |
| 15  | [Warp up](./notes/14-warp-up.md)                                                          | ⏳           |
